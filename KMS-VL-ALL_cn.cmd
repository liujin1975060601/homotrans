...
:: 获取已安装的操作系统架构版本；操作系统语言环境独立于 Windows XP / Server 2003 及以上版本
...
:: 定义已安装版本为Windows 10 -1607- 及以上版本
if %_WinBuild% LSS 14393 goto :Main
:: 获取基于活动 CBS 包的版本，或者退回到 Dism 当前版本
...
if %_WinBuild% GEQ 9600 (
  REM 在 echo 消息中没有附带内容或括号
  echo 当前操作系统：Windows 8.1 或更高版本
  goto :Win8.1AndLater
) else if %_WinBuild% GEQ 2600 (
  echo 当前操作系统：Windows 8 或更低版本
  goto :Win8AndEarlier
) else (
...
if %_OSPS% NEQ 0 (
  if %_OfflineMode% EQU 1 (
    REM 本地主机 IP 应用于 Windows 8.1 及更高版本的 Office 2010
...
::==========================================
:Win8AndEarlier
::  如果没有 Office 2010 产品被安装在 Windows XP SP3/Server 2003 R2 上，程序将自动退出
if %_OSPS% EQU 0 (
  if %_WinBuild% LSS 6000 (
    echo.
    echo 没有检测到 Office 2010 产品...
    call :Close & exit /b
  )
)
...
if %_OfflineMode% EQU 1 (
  ...
  set "_KMSHost=%_KMSLocalHost%"
  REM Windows Vista 不支持 StIKEngEngestServices 端口，因此恢复到默认的 KMS 端口
...
::==========================================
:AddFirewallRule
:: 添加 VLMCSD 的 KMS 规则到 Windows 防火墙
if %_WinBuild% LSS 6000 (
...
:: 令人难以置信的 BUG 修复；Windows Vista 及以下版本需要一些时间才能启动 KMS 服务器防止激活；所以为它添加延迟以成功启动
if %_WinBuild% LSS 7600 (
...
wmic path %_MicrosoftProduct% where (Description like '%%KMSCLIENT%%') get Name /value %_Nul_2% | findstr /i "Office" %_Nul_1% && (
  ...
  if %_WinBuild% GEQ 9200 (
    echo.
    echo 没有检测到 Office 2013 及以上版本的 VL 产品.
  )
)

:: 检查已安装的 Windows 是否支持 KMS 激活；如果没有 VL 的 Windows 或 Office 则退出
wmic path %_MicrosoftProduct% where (Description like '%%KMSCLIENT%%') get Name /value %_Nul_2% | findstr /i "Windows" %_Nul_1% || (
  echo.
  echo 没有检测到 Windows VL 版本.
  if %_OfficeVL% EQU 0 (
    exit /b
  )
)
...
wmic path %_MicrosoftProduct% where (Description like '%%KMSCLIENT%%') get Name %_Nul_1_2% || (
  if %_WinBuild% LSS 9200 (
    echo.
    echo 没有检测到 Office 2010 及以上版本 VL 产品.
    exit /b
  ) else (
    echo.
    echo 没有检测到 Office 2010 VL 产品.
    exit /b
  )
)
...
:: 这仅适用于 SoftwareLicensingService 版本 6.2 及更高版本；不适用于 OfficeSoftwareProtectionService
if %_WinBuild% GEQ 9200 (
...
if %_OfflineMode% EQU 1 (
  ...
  REM 这仅适用于 SoftwareLicensingService 版本 6.2 及更高版本；不适用于 OfficeSoftwareProtectionService
  if %_WinBuild% GEQ 9200 (
...
:: 如果检测到 KMS 客户端已经提前激活或具有 GVLK ，则调用激活功能
...
:: 对于多版本 Windows 10 SKU-IDs 程序将不再执行
for %%G in (
...
:: 如果检测到的 KMS 客户端没有 GVLK，将检查永久激活，然后安装 GVLK 并激活它
...
wmic path %_MicrosoftProduct% where (LicenseStatus='1' and GracePeriodRemaining='0') get Name %_Nul_2% | findstr /i "Windows" %_Nul_1% && (
  echo.
  echo 检测到 Windows %_ProductName% 已被永久激活.
  exit /b
)

:: 如果 Windows 没有永久激活，则安装 GVLK 并激活
call :SelectKey %1
exit /b
::==========================================
:CheckOffice19
:: 检查 Office 2019 Pro SKUs 之前的许可证
...
if "%_License2%" EQU "1" (
  echo 检测到 %5 已被永久 MAK 激活.
  exit /b
)
if "%_License%" EQU "1" (
  echo 检测到 %3 已被永久 MAK 激活.
  exit /b
)
:: 如果 Office 产品没有被永久激活，则安装 GVLK 并激活
call :SelectKey %1
exit /b
::==========================================
:OfficeDetect
...
::==========================================
:VisualStudio
:: 如果安装了多个 Visual Studio 版本，则清除变量以避免冲突
set "_VS="
...
start "" /b "%_VS%\StorePID.exe" %~4 %~5 && (
  echo Visual Studio %~6 激活成功.
  echo.
) || (
  echo Visual Studio %~6 激活失败.
  echo.
)
exit /b
::==========================================
...
if %_gprMinutes% EQU 43200 (
  if %_WinBuild% EQU 9200 if %_OfficeSLP% EQU 0 (
    echo Windows 8 核心版/专业版（含 WMC）激活成功
...
echo.
echo GVLK 为 %_Name%
echo 使用 SKU-ID %1 没有找到
echo.
echo 如果本次激活失败，请手动为该产品输入对应的 GVLK 并重新运行 KMS_VL_ALL
call :Activate %1
exit /b
::==========================================
:InstallKey
